...
from smartpihome.models import Light, RGBLight, RaspberryPi, TemperatureSensor, GPA, HumiditySensor
import time, pigpio
...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		...
		#init humiditySensors
		db_humidities = HumiditySensor.objects.all()
		hums = {}
		for hum in db_humidities:
			if pis[hum.pi.name].connected:
				hums[hum.name] =  Humidity()
		...
		while True:
			...
			#handle Hum Sensors
			sens = HumiditySensor.objects.all()
			for sen in sens:
				hum = hums[sen.name].read(sen.channel)
				print("Humidity:", hum)
				group_message = json.dumps({"value": str(hum), "sensor_name" : str(sen.name)})
				Group("sensor").send({"text" : group_message})
			time.sleep(0.5)
